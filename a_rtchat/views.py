...
@login_required
def send_invite(request, chatroom_name):
    chat_group = get_object_or_404(ChatGroup, group_name=chatroom_name)
    if request.user != chat_group.admin:
        raise Http404()
    ...
    if request.method == 'POST':
        form = ChatInvitationForm(request.POST)
        if form.is_valid():
            invited_username = form.cleaned_data['username']
            try:
                invited_user = User.objects.get(username=invited_username)
                if invited_user not in chat_group.members.all():
                    chat_group.members.add(invited_user) 
                ...
                messages.error(request, f'User "{invited_username}" does not exist.')
    ...
    return render(request, 'a_rtchat/chat_send_invitation.html', context)
